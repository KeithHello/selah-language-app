...
void main() {
  test('starter seed bundle contains ten balanced bilingual sentences', () {
    ...
    expect(sentences.map((sentence) => sentence['id']), [
      'seed-001',
      'seed-006',
      'seed-027',
      'seed-012',
    ...
    expect(sentences.map((sentence) => sentence['category']), [
      'work',
      'friends',
      'daily_life',
      'vent',
    ...
    final japaneseSeed = LearnSentence.seed({
      ...sentences.firstWhere((sentence) => sentence['id'] == 'seed-006'),
      'source': sentences.firstWhere(
  ...
  test(
    'starter audio manifest has four English voices and both native sources',
    () {
      final seedFile = File('../SeedContent/seed-sentences.json');
      final seedJson =
          jsonDecode(seedFile.readAsStringSync()) as Map<String, dynamic>;
      final sentences = (seedJson['sentences'] as List)
      ...
      for (final seed in seedIds) {
        ...
        expect(
          manifest['$seed:source:zh-Hant'],
          isNotNull,
          reason: '$seed Chinese source',
        );
        expect(
          manifest['$seed:source:ja'],
          isNotNull,
          reason: '$seed Japanese source',
        );
      }
  ...
  test('web bundle declares only active starter audio files', () {
    final manifest =
        jsonDecode(File('assets/content/seed-audio.json').readAsStringSync())
            as Map<String, dynamic>;
    final pubspec = File('pubspec.yaml').readAsStringSync();

    expect(
      RegExp(r'^\s*-\s*assets/audio/\s*$', multiLine: true).hasMatch(pubspec),
      isFalse,
    );
    for (final entry in manifest.values) {
      final audio = entry as Map<String, dynamic>;
    ...
    final declaredFiles =
        RegExp(r'^\s+- assets/audio/(.+\.mp3)\s*$', multiLine: true)
            .allMatches(pubspec)
            .map((match) => 'assets/audio/${match.group(1)}')
            .toList();
    final manifestFiles = manifest.values
        .map((entry) => (entry as Map<String, dynamic>)['path'])
        .toList();
    expect(declaredFiles..sort(), manifestFiles.cast<String>()..sort());
  });
}
