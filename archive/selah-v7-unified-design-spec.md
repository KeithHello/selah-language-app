# Selah v7 Unified Design Spec

> Status: source of truth for development planning.
> Date: 2026-07-02
> Basis: v7 minimal direction. Older PRD, system-design, pet-system, architecture, and v4-v6 prototype files are historical references unless this document explicitly keeps a mechanism.
...
This is the v7 learning philosophy:
...
2. Sentence-care list: which sentences the system is quietly carrying forward.
...
- A small "小豆正在照顧的句子" area showing 2-3 sentence statuses.
...
- Choose target language. For v7 development, English is the only production target; Japanese can be visible only if the team is ready to explain it as future / preview.
- Name the sprite.
...
- The preferred v7 interaction is a single immersive sentence card that naturally changes state.
...
Preferred v7 Listen interaction:
...
- The older 8-10 sentence plan is not required for v7. It can return later if engagement data supports it.
...
V7 keeps the learning intent of Smart Excel but not its visible complexity.
...
- Practice should show 3 sentences per set in early v7.
...
This is the v7 replacement for visible Smart Excel. Do not expose state names, due dates, red/green labels, or levels to users.

## 10. AI And Audio Requirements

V7 requires real AI/audio in production, even if the prototype simulates them.
...
- Not required during first onboarding for v7.
...
## 13. What To Retire For V7

Retired for v7 development:
...
These may be revisited only after the v7 loop proves retention and learning value.
...
5. Are old PRD features clearly excluded from v7 scope?
...
Current answer after reviewing the v7 prototype: mostly yes, with the improvement items listed in `selah-v7-prototype-audit.md`.
