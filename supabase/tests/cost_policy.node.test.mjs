...
test('transcription cost calculation respects ms rates and ceiling', () => {
  ...
  assert.throws(() => calculateOperationMaxCost('transcription', { durationMs: 180_001 }));
...
test('addCalendarMonthsUtc handles month-end anchors and leap years cleanly', () => {
  ...
  assert.deepStrictEqual(periods[0].startsAt.toISOString(), '2026-01-31T10:00:00.000Z');
  assert.deepStrictEqual(periods[0].endsAt.toISOString(), '2026-02-28T10:00:00.000Z');
  assert.deepStrictEqual(periods[1].startsAt.toISOString(), '2026-02-28T10:00:00.000Z');
  assert.deepStrictEqual(periods[1].endsAt.toISOString(), '2026-03-31T10:00:00.000Z');
  assert.deepStrictEqual(periods[2].startsAt.toISOString(), '2026-03-31T10:00:00.000Z');
  assert.deepStrictEqual(periods[2].endsAt.toISOString(), '2026-04-30T10:00:00.000Z');
});
