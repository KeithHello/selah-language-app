...
Deno.test("trial activation requires a persisted successful personal result", () => {
  ...
  assertStringIncludes(
    SQL,
    "GRANT EXECUTE ON FUNCTION public.activate_trial_with_result",
  );
});
...
Deno.test("admin membership actions are idempotent and audited in one RPC", () => {
  ...
  assertStringIncludes(
    body,
    "status IN ('reserved', 'dispatch_claimed', 'settled', 'unknown')",
  );
  ...
  assertStringIncludes(
    body,
    "NULLIF(btrim(COALESCE(p_transaction_id, v_order.channel_transaction_id)), '')",
  );
  assertStringIncludes(
    SQL,
