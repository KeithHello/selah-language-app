...
test("maps missing dates to preparing instead of active", () => {
  ...
  assert.equal(
    trialStateFromMembership({
      plan: "trial",
      status: "trial",
      periodStartsAt: "2026-09-12T00:00:00Z",
      periodEndsAt: "2026-09-19T00:00:00Z",
    }),
    "active",
  );
  assert.deepEqual(TRIAL_STATES, [
    "not_started",
    "preparing",
    "active",
    "expired",
...
test("uses a half-open server window for active trial access", () => {
  ...
  assert.equal(
    isTrialWindowOpen(start, end, new Date("2026-09-12T00:00:00Z")),
    true,
  );
  assert.equal(
    isTrialWindowOpen(start, end, new Date("2026-09-19T00:00:00Z")),
    false,
  );
});
