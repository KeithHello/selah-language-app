...
function deps(overrides: Partial<UserResearchProfileDependencies> = {}) {
  ...
  const dependencies: UserResearchProfileDependencies = {
    requireAuth: () => USER_ID,
    env: { get: (name: string) => name === "SUPABASE_URL" ? "https://example.test" : "service-key" },
    createSupabase: () => ({
...
function request(method: string, body?: Record<string, unknown>) {
  return new Request("https://example.test/functions/v1/user-research-profile", {
    method,
...
Deno.test("reads only the authenticated user's research profile", async () => {
  const setup = deps();
  const response = await createUserResearchProfileHandler(setup.dependencies)(request("GET"));
...
Deno.test("rejects saving without explicit consent before calling the database", async () => {
  const setup = deps();
  const response = await createUserResearchProfileHandler(setup.dependencies)(request("POST", {
    operation: "save",
