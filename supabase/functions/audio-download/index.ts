...
import { errorResponse, handleOptions, requireAuth } from "../_shared/cors.ts";
...
Deno.serve(async (req: Request) => {
  ...
  const rangeCheck = parseSingleByteRange(
    rangeHeader,
    manifest.byte_size || 1024 * 1024,
  );
  if (!rangeCheck.valid) {
    return errorResponse(
